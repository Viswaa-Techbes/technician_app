...
async function seed() {
  try {
    ...
    console.log('Connected to MongoDB for CCTV Seeding');
    ...
    for (const bName of brandsData) {
      ...
      if (!b) {
        ...
        console.log(`Seeded Brand: ${bName}`);
      }
      seededBrands[bName] = b._id;
    }

    // 2. Seed Models & Camera Pricing
    // CP Plus / Secureye models
    ...
    async function insertModels(brandId, modelList) {
      for (const m of modelList) {
        ...
        if (!existing) {
          ...
          console.log(`Seeded Model: ${m.cameraType} - ${m.name} (${m.resolution})`);
        }
      }
    }

    await insertModels(seededBrands['CP Plus'], cpSecureyeModels);
    await insertModels(seededBrands['Secureye'], cpSecureyeModels);
    await insertModels(seededBrands['Mikam'], mikamModels);
    await insertModels(seededBrands['Hikvision'], hikvisionModels);
    ...
    for (const sd of sdCardsData) {
      ...
      if (!existing) {
        ...
        console.log(`Seeded SD Card: ${sd.capacity} -> ₹${sd.price}`);
      }
    }
    ...
    for (const c of cablesData) {
      ...
      if (!existing) {
        ...
        console.log(`Seeded Cable: ${c.name} -> ₹${c.price}`);
      } else {
        existing.price = c.price;
        await existing.save();
        console.log(`Updated Cable: ${c.name} -> ₹${c.price}`);
      }
    }
    ...
    for (const ic of installationChargesData) {
      ...
      if (!existing) {
        ...
        console.log(`Seeded Installation Charge: ${ic.name} -> ₹${ic.price}`);
      }
    }
    ...
    for (const acc of accessoriesData) {
      ...
      if (!existing) {
        ...
        console.log(`Seeded Accessory: ${acc.name} -> ₹${acc.price}`);
      }
    }
    ...
    for (const h of hddsData) {
      ...
      if (!existing) {
        ...
        console.log(`Seeded HDD: ${h.capacity} -> ₹${h.price}`);
      }
    }
    ...
    for (const r of racksData) {
      ...
      if (!existing) {
        ...
        console.log(`Seeded Rack: ${r.type} -> ₹${r.price}`);
      }
    }
    ...
    if (!config) {
      await CctvPricingConfig.create({
        baseCharge: 499, // Visit Charges
        indoorCharge: 0,
        outdoorCharge: 350,
        wirePricePerMeter: 60, // CAT6 cable default
        tax: { label: 'GST', percentage: 18, status: 'active' },
        status: 'active',
      });
      console.log('Seeded Default CctvPricingConfig');
    }

    console.log('CCTV Seeding completed successfully!');
    process.exit(0);
  } catch (err) {
